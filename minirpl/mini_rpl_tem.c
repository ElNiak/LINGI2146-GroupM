...
{
    printf("SENDER RCV TEM - runicast message received from %d.%d, seqno %d\n",
           from->u8[0], from->u8[1], seqno);
    printf("SENDER RCV TEM - packetbuf_dataptr = %s\n",
           (char *)packetbuf_dataptr());
    if (parent.node_addr.u8[0] != 0)
    {
        while(runicast_is_transmitting(&runicast)){}
        packetbuf_copyfrom(packetbuf_dataptr(), strlen(packetbuf_dataptr()));
	printf("SENDER SNT R TEM - %u.%u: sending runicast to address %u.%u\n",
                    from->u8[0],
                    from->u8[1],
...
{
    printf("SENDER TEM - runicast message sent to %d.%d, retransmissions %d\n",
           to->u8[0], to->u8[1], retransmissions);
}
...
{
    printf("SENDER TEM - runicast message timed out when sending to %d.%d, retransmissions %d\n",
           to->u8[0], to->u8[1], retransmissions);
}
...
PROCESS_THREAD(rime_sender_process, ev, data)
{
    ...
    if(rimeaddr_node_addr.u8[0] == 1 &&
        rimeaddr_node_addr.u8[1] == 0) {
            printf("TEM PROCESS_WAIT_EVENT_UNTIL\n");
            PROCESS_WAIT_EVENT_UNTIL(0);
    }

    if(parent.node_addr.u8[0] == 0 &&
        parent.node_addr.u8[1] == 0) {
            printf("TEM PROCESS_WAIT_EVENT_UNTIL 2\n");
            PROCESS_WAIT_EVENT_UNTIL(0);
    }

    printf("TEM PROCESS_WAIT_EVENT_UNTIL 3\n");
    while(1)
    {
        static struct etimer et;
        etimer_set(&et,4 *CLOCK_SECOND);
        ...
        if(!runicast_is_transmitting(&runicast2)) {
            ...
            char * pp = generateDataT(outData);
            printf("SENDER SNT TEM - payload = %s \n",pp);
            packetbuf_copyfrom(pp, strlen(pp));
            printf("SENDER SNT TEM - %u.%u: sending runicast to address %u.%u\n",
